		...
		namespace Mesh {
			...
			BVHNode * BuildRecursively(TriangleBoundingBoxEntry & Boxes, int &Index, int Depth = 0) {
				...
				for (int Axis = 0; Axis < 3; Axis++) {
					...
					for (float TestSplit = Start + Step; TestSplit < Stop - Step; TestSplit += Step) {
						...
						LDEngine::Core::BoundingBox Right, Left; 
				...
				LDEngine::Core::BoundingBox LeftBox, RightBox; 
			...
			BVHNode * ConstructBoundingVolumeHieracy(Mesh & Mesh)	{
				...
				for (int Triangle = 0; Triangle < Mesh.Vertices.size(); Triangle += 3) {
					...
					for (int Axis = 0; Axis < 3; Axis++) {
						TemporaryEntry.Max[Axis] = LDEngine::Core::max3(Mesh.Vertices[Triangle][Axis], Mesh.Vertices[Triangle + 1][Axis], Mesh.Vertices[Triangle + 2][Axis]);
						TemporaryEntry.Min[Axis] = LDEngine::Core::min3(Mesh.Vertices[Triangle][Axis], Mesh.Vertices[Triangle + 1][Axis], Mesh.Vertices[Triangle + 2][Axis]);
						//Node->Max[Axis] = LDEngine::Core::max4(Node->Max[Axis], Mesh.Vertices[Triangle][Axis], Mesh.Vertices[Triangle + 1][Axis], Mesh.Vertices[Triangle + 2][Axis]);
						//Node->Min[Axis] = LDEngine::Core::min4(Node->Max[Axis], Mesh.Vertices[Triangle][Axis], Mesh.Vertices[Triangle + 1][Axis], Mesh.Vertices[Triangle + 2][Axis]);
